...
serve(async (req: Request) => {
  ...
  if (!jsonStr) {
    console.error('[bid-no-bid-analysis] no balanced JSON object found — stop_reason:', claudeData.stop_reason, rawText.slice(0, 1000))
    return respond({ error: claudeData.stop_reason === 'max_tokens'
      ? 'AI response was truncated before it finished — try again'
